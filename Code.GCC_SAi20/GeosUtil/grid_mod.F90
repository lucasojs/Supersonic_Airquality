...
#if defined( MODELE )
          ! ModelE grid is not centered on 180
          ! Longitude edges
          XMD(I,J,L)  = LON_DG(I,1) ! From GEOM_B.f
...
# elif defined( MODELE )
          ...
          !         %%%%%%% GEOS-Chem CLASSIC (with OpenMP) %%%%%%%
          !
          ! For GISS ModelE, there are no half-size polar boxes.
          ! Compute the latitude centers accordingly.  (bmy, 7/2/13)
          !----------------------------------------------------------------

          ! Lat centers (degrees)
          YMD(I,J,L)     = LAT_DG(J,1) ! From GEOM_B.f

#  ifdef GRIDF40
          ! GISS convention at poles is different than GEOS-Chem/TPCORE
          IF ( J .eq. JSP ) YMID(I,J,L) = -89d0
          IF ( J .eq. JNP ) YMID(I,J,L) =  89d0
#  endif
...
# else
          ...
          ! the polar grid boxes (for non-GISS global grids only) be
...
# endif
          ...
          ! Lat edges (degrees)
          YDG(I,J,L) = LAT_DG(J,2) ! From GEOM_B.f
