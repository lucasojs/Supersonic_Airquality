...
!
  ...
  TYPE, PUBLIC :: MetState
     ...
     REAL(fp), POINTER :: PSC1      (:,:  ) ! Interpolated sfc pressure [hPa]
     REAL(fp), POINTER :: PSCM      (:,:  ) ! Interpolated sfc pressure [hPa]
...
!                                            !  5 hrs ago (for PARANOX)
     ...
     REAL(fp), POINTER :: BXHEIGHT  (:,:,:) ! Grid box height [m]
     REAL(fp), POINTER :: CLDF      (:,:,:) ! 3-D cloud fraction [1]
     REAL(fp), POINTER :: CMFMC     (:,:,:) ! Cloud mass flux [kg/m2/s]
     REAL(fp), POINTER :: C_ED1     (:,:,:)  ! Entrainment by DD1 [Pa/s]
     REAL(fp), POINTER :: C_ED2     (:,:,:)  ! Entrainment by DD2 [Pa/s]
     REAL(fp), POINTER :: C_EU1     (:,:,:)  ! Entrainment by UD1 [Pa/s]
     REAL(fp), POINTER :: C_OD1     (:,:,:)  ! Outflow by DD1 [Pa/s]
     REAL(fp), POINTER :: C_OD2     (:,:,:)  ! Outflow by DD2 [Pa/s]
     REAL(fp), POINTER :: CON_OD_TOT(:,:,:) ! Contrail OD (linear) [1]      
     REAL(fp), POINTER :: CON_FRC   (:,:,:) ! 3-D contrail cloud fraction [1]      
     REAL(fp), POINTER :: CON_SAD   (:,:,:) ! Contrail SAD [m2/m3]      
     REAL(fp), POINTER :: CON_RVOL  (:,:,:) ! Contrail vol-weighted radius [m]      
     REAL(fp), POINTER :: CON_RAREA (:,:,:) ! Contrail area-weighted radius [m]      
     REAL(fp), POINTER :: CON_VOL   (:,:,:) ! Contrail mixing ratio [m3/m3]
     REAL(fp), POINTER :: CON_NDENS (:,:,:) ! Contrail number density [#/m3]
...
!
    ...
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%PSC1     = 0.0_fp

    ALLOCATE( State_Met%PSCM      ( IM, JM ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%PSCM     = 0.0_fp
...
!    State_Met%SUNCOSmid5 = 0.0_fp
    ...
    ! SDE 2015-05-26: Contrail fields
    ALLOCATE( State_Met%CON_FRC   ( IM, JM, LM ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%CON_FRC  = 0.0_fp

    ALLOCATE( State_Met%CON_RVOL  ( IM, JM, LM ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%CON_RVOL = 0.0_fp

    ALLOCATE( State_Met%CON_RAREA ( IM, JM, LM ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%CON_RAREA= 0.0_fp

    ALLOCATE( State_Met%CON_NDENS ( IM, JM, LM ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%CON_NDENS= 0.0_fp

    ALLOCATE( State_Met%CON_SAD   ( IM, JM, LM ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%CON_SAD  = 0.0_fp

    ALLOCATE( State_Met%CON_OD_TOT( IM, JM, LM ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%CON_OD_TOT= 0.0_fp

    ALLOCATE( State_Met%CON_VOL   ( IM, JM, LM ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%CON_VOL  = 0.0_fp

#if defined( GCAP ) || defined( GISS )
    ...
    ALLOCATE( State_Met%HLAKE     ( IM, JM ), STAT=RC )
    IF ( RC /= GIGC_SUCCESS ) RETURN
    State_Met%HLAKE    = 0.0_fp
...
!    IF ( ASSOCIATED( State_Met%SUNCOSmid5 )) DEALLOCATE( State_Met%SUNCOSmid5 )
    ...
    IF ( ASSOCIATED( State_Met%CON_RVOL   )) DEALLOCATE( State_Met%CON_RVOL   )
    IF ( ASSOCIATED( State_Met%CON_RAREA  )) DEALLOCATE( State_Met%CON_RAREA  )
    IF ( ASSOCIATED( State_Met%CON_NDENS  )) DEALLOCATE( State_Met%CON_NDENS  )
    IF ( ASSOCIATED( State_Met%CON_VOL    )) DEALLOCATE( State_Met%CON_VOL    )
    IF ( ASSOCIATED( State_Met%CON_SAD    )) DEALLOCATE( State_Met%CON_SAD    )
    IF ( ASSOCIATED( State_Met%CON_OD_TOT )) DEALLOCATE( State_Met%CON_OD_TOT )
    IF ( ASSOCIATED( State_Met%CON_FRC    )) DEALLOCATE( State_Met%CON_FRC    )
