...
!
  ...
  ! Use 6.3 Tg N a-1 for PD, which is about 310 mol/flash
  REAL*8,  PARAMETER   :: RFLASH_MIDLAT = 6.3d12 * 6.022d23 / 14d0 / &
                              ( 45.8658d0 * 3600d0 * 24d0 * 365.25d0 )
  REAL*8,  PARAMETER   :: RFLASH_TROPIC = RFLASH_MIDLAT
#else
...
!$OMP SCHEDULE( DYNAMIC )
    ...
    DO I = 1, HcoState%NX
       ...
       GLOB_FR = GLOB_FR + RATE / 6.d0 / 3600.d0 ! Flashes / second
...
!$OMP END PARALLEL DO
    ...
    IF ( DoDiagn ) THEN
       DiagnID =  56001
       Arr2D   => DIAGN(:,:,1)
       CALL Diagn_Update( am_I_Root, HcoState, ExtNr=ExtNr,cID=DiagnID, Array2D=Arr2D, RC=RC) 
       IF ( RC /= HCO_SUCCESS ) RETURN 
       Arr2D => NULL() 
    ...
    !WRITE(6,*) 'Global flash rate (fl s-1): ', GLOB_FR
...
!
    ...
    !=================================================================
#if   defined( GRID2x25      ) || defined( GRIDF40 )
    REAL*8, PARAMETER     :: ANN_AVG_FLASHRATE = 45.8650d0

#elif defined( GRID4x5       ) || defined( GRIDM23 ) || defined( GRIDM40 )
...
#elif defined( GISS ) && defined( GRIDM23 ) || defined( GRIDM40 )

      !---------------------------------------
      ! GISS M23/40 global simulation
      !---------------------------------------
      BETA = ANN_AVG_FLASHRATE / 6.5588969d0

#elif defined( GISS ) && defined( GRIDF40 )

      !---------------------------------------
      ! GISS F40 global simulation
      !---------------------------------------
      BETA = ANN_AVG_FLASHRATE / 34.92472513949209655947925d0
...
#ifdef GISS
       WRITE( *,* ) 'The lightning code must be tuned for every'
       WRITE( *,* ) 'new climate system. Recommended method:' 
       WRITE( *,* ) '(1) Run present-day simulation of GCM with the'
       WRITE( *,* ) '    same version of the code as you are using'
       WRITE( *,* ) '    for whatever period you are looking at.'
       WRITE( *,* ) '(2) Uncomment the CALL GEOS_CHEM_STOP trap in'
       WRITE( *,* ) '    GeosCore/lightning_nox_mod.F, and run'
       WRITE( *,* ) '    GEOS-Chem for the whole period with'
       WRITE( *,* ) '    only lightning emissions (no transport,'
       WRITE( *,* ) '    chemistry, other emissions, etc.), which'
       WRITE( *,* ) '    is fast.'
       WRITE( *,* ) '(3) Calculate long-term global annual mean '
       WRITE( *,* ) '    lightning flash rate, FR from ND56 diagn' 
       WRITE( *,* ) '(4) Set BETA in GeosCore/lightning_nox_mod.F'
       WRITE( *,* ) '    to be:'
       WRITE( *,* ) '           BETA = ANN_AVG_FLASHRATE / FR    '
       WRITE( *,* ) '    You should expect about 6 Tg N a-1 in the'
       WRITE( *,* ) '    present day.'
       WRITE( *,* ) '(5) You may now use this BETA value for any past'
       WRITE( *,* ) '    or future climate scenarios that uses the'
       WRITE( *,* ) '    same version of the GCM code.'
       WRITE( *,* ) 'You may contact Lee Murray with any questions.'
#else
