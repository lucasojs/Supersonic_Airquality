...
ERR_MET              :="Select a met field: MET=gcap, MET=geos4, MET=geos5, MET=merra, MET=geos-fp, MET=ModelE, MET=Model3)"

# Error message for bad GRID input
ERR_GRID             :="Select a horizontal grid: GEOS: GRID=4x5, GRID=2x25, GRID=05x0666, GRID=025x03125; GISS: GRID=M23, GRID=F40, GRID=M40"
...
ifndef NO_MET_NEEDED 
  ...
  REGEXP             :=(\-DGCAP|\-DGEOS_4|\-DGEOS_5|\-DMERRA|\-DGEOS_FP|\-DMODEL3|\-DMODELE)
  ifneq ($(shell [[ "$(USER_DEFS)" =~ $(REGEXP) ]] && echo true),true)
    $(error $(ERR_MET))
  endif
...
ifndef NO_GRID_NEEDED
  ...
  ifeq ($(GISS_NEEDED),1)
    ...
    # M40 is GISS medium (4x5) horizontal with 40 vertical layers
    REGEXP         :=(^[Mm]40|^[Mm].40|(^4.5|^4\.0.5\.0))
    ifeq ($(shell [[ "$(GRID)" =~ $(REGEXP) ]] && echo true),true)
      USER_DEFS      += -DGRIDM40
    endif
  ...
  REGEXP             := ((\-DGRID)?M23|M40|F40|4x5|2x25|1x125|05x0666|025x03125)
  ifneq ($(shell [[ "$(USER_DEFS)" =~ $(REGEXP) ]] && echo true),true)
    $(error $(ERR_GRID))
  endif
