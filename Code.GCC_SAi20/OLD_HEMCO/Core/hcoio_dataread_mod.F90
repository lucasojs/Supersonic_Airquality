...
!
    ...
    REAL*8,  POINTER      :: availYMDhm(:) => NULL() 
    LOGICAL               :: ExitSearch 
    LOGICAL               :: verb
    INTEGER               :: STAT
    ...
    ! Chop off minutes and convert to integer
    allocate(availYMDH(Size(availYMDHm)),STAT=STAT)
    If (STAT.ne.0) Then
       CALL HCO_ERROR( 'Could not allocated availYMDH', STAT )
       RETURN 
    End If
    availYMDH = INT(FLOOR(availYMDHm/100))
