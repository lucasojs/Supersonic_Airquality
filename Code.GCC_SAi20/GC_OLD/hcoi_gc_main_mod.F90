...
!------------------------------------------------------------------------------
    ...
    !-----------------------------------------------------------------------
    ! NON-EMISSIONS DATA #6: Contrail cirrus
    !
    ! Set the UV albedo toggle according to options in HEMCO_Config. This 
    ! will enable/disable all fields in HEMCO_Config that are  bracketed by 
    ! '+CONTRAIL+'.  Check first if this bracket values has been set 
    ! explicitly in the HEMCO configuration file, in which case it will
    ! not be changed.
    !
    ! Now search through full list of extensions (ExtNr = -999).
    !-----------------------------------------------------------------------
    CALL GetExtOpt( -999, '+CONTRAIL+',  OptValBool=LTMP, &
                            FOUND=FOUND, RC=RC )
    IF ( RC /= HCO_SUCCESS ) THEN
       CALL ERROR_STOP( 'GetExtOpt +CONTRAIL+', LOC )
    ENDIF
...
!       ! Stop the run if this collection is defined in the HEMCO config
!       ! file, but is set to an value inconsistent with input.geos file.
!       IF ( Input_Opt%LCONTRAIL .AND. ( .NOT. LTMP ) ) THEN
!          MSG = 'Setting +CONTRAIL+ in the HEMCO configuration file ' // &
!                'must not be disabled if contrails are turned on. '   // &
!                'If you don`t set that setting explicitly, it will '  // &
!                'be set automatically during run-time (recommended)'
!          CALL ERROR_STOP( MSG, LOC ) 
!       ENDIF
    ...
    ELSE
       ...
       IF ( Input_Opt%ITS_A_FULLCHEM_SIM   .or. &
            Input_Opt%ITS_AN_AEROSOL_SIM ) THEN
          IF ( Input_Opt%LCHEM ) THEN
             OptName = '+CONTRAIL+ : true'
          ELSE
             OptName = '+CONTRAIL+ : false'
          ENDIF
       ELSE
          OptName = '+CONTRAIL+ : false'
       ENDIF
       CALL AddExtOpt( TRIM(OptName), CoreNr, RC )
       IF ( RC /= HCO_SUCCESS ) THEN
          CALL ERROR_STOP( 'AddExtOpt +CONTRAIL+', LOC )
       ENDIF
